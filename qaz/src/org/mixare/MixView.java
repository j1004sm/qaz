...
public class MixView extends Activity implements SensorEventListener,
	...
	public static MixContext mixContext; // 메인 컨텍스트
...
//			}
			...
			if (settings.getBoolean("firstAccess", false) == false) {
				...
				dataSourceEditor
						.putString("DataSource0",
								"위키피디아|http://ws.geonames.org/findNearbyWikipediaJSON|0|0|true");
				dataSourceEditor
						.putString("DataSource1",
								"트위터|http://search.twitter.com/search.json|2|0|false");
				dataSourceEditor
						.putString(
								"DataSource2",
				...
				dataSourceEditor
						.putString("DataSource3",
								"드로우온리얼|http://www.manjong.org:8255/qaz/check.jsp|4|0|true");
	...
	private SeekBar.OnSeekBarChangeListener myZoomBarOnSeekBarChangeListener = new SeekBar.OnSeekBarChangeListener() {
		...
		public void onProgressChanged(SeekBar seekBar, int progress,
			...
			// 변경 사항을 출력해 준다
			t.setText("반경 " + String.valueOf(myout) + "km 이내 검색");
			t.show();
		}
	...
	public boolean onTouchEvent(MotionEvent me) {
		if (mGestures != null) {
			return mGestures.onTouchEvent(me);
		} else {
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
		try {
			...
			try {
				...
				// 프리뷰 형태의 팩터
				float ff = (float) w / h;
				Log.d("Qaz-Mixare", "Screen res: w:" + w + " h:" + h
						+ " aspect ratio:" + ff);
				...
				while (itr.hasNext()) {
					...
					// 각 요소들의 변수값을 로그로 기록
					Log.d("Qaz-Mixare", "Candidate camera element: w:"
					...
					if ((cff >= ff)
							&& ((element.width >= bestw) && (element.width >= besth))) {
						bff = cff;
						bestw = element.width;
				...
				Log.d("Qaz-Mixare", "Chosen camera element: w:" + bestw + " h:"
